...
type (
	ISaleableProductService interface {
		FindAllSaleableProducts(company_id string, categoryQuery []string, searchingProduct string) (saleableProduct *[]Response.SaleableResponseDTO, err error, statusCode int)
	}
...
func (s *SaleableProductService) FindAllSaleableProducts(company_id string, categoryQueries []string, searchingProduct string) (saleableProduct *[]Response.SaleableResponseDTO, err error, statusCode int) {
	...
	if len(categoryQueries) == 0 && searchingProduct == "" {
	...
	saleableProductInit, err := s.SaleableProductRepository.FindByName(company_id, searchingProduct, categoryIDs)
	if err != nil {
		return nil, err, http.StatusInternalServerError
	}
